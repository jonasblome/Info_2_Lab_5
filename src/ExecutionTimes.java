...
//		executor.fragment8(100000);
		...
		System.out.println("20 bit numbers:");
		checkIsPrimeRuntime(100, 20);
		System.out.println();
		
		System.out.println("40 bit numbers:");
		checkIsPrimeRuntime(100, 40);
		System.out.println();
	}
	...
	public static void checkIsPrimeRuntime(int n, int bitSize) {
		long start;
		long end;
		long duration;
		...
		int min = 0;
		int max = (int) Math.pow(2,  bitSize-1);
		
		for(int i = 0; i < n; i++) {
			Random randomGenerator = new Random();
			
			int result = randomGenerator.nextInt(max - min) + min;
			isPrime(result);
		}
